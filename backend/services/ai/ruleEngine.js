/**
 * @file ruleEngine.js
 * @description Deterministic rule-based scam analysis engine for SurakshaAI.
 * Evaluates text messages, UPI IDs, QR code payloads, and voice DSP metrics using
 * domain whitelists, regex boundary matchers, and calibrated risk scoring.
 */
...
class RuleEngine {
  /**
   * Analyzes text messages for phishing, impersonation, digital arrest, and scam patterns.
   * @param {string} text 
  ...
  /**
   * Validates UPI IDs against provider extensions and brand whitelists.
   * @param {string} upiId 
   * @returns {Object} Structured UPI analysis output
   */
  analyzeUPI(upiId) {
  ...
  /**
   * Analyzes QR code payloads and UPI QR URIs.
   * @param {string} url 
   * @returns {Object} Structured QR analysis output
   */
  analyzeQR(url) {
    ...
    if (url.startsWith('upi://')) {
      ...
      const upiResult = this.analyzeUPI(pa);
      return {
        ...upiResult,
  ...
  /**
   * Analyzes client-side DSP voice features or provides calibrated fallback scenarios.
   * @param {Object} features Acoustic features object
   * @returns {Object} Structured voice analysis output
   */
  analyzeVoice(features) {
